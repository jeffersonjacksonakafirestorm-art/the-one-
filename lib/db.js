/**
 * Database client
 * Uses Supabase when configured, falls back to in-memory store for dev/preview.
 */
...
// In-memory fallback — resets on redeploy, dev use only
const memStore = {
  businesses: [],
  missed_calls: [],
  conversations: [],
...
export async function updateBusiness(id, updates) {
  ...
  memStore.businesses[idx] = { ...memStore.businesses[idx], ...updates };
